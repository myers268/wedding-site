...
CREATE TABLE `__new_event_attendance` (
	...
	`attending` text DEFAULT 'DEFAULT' NOT NULL,
	CONSTRAINT "attending_enum" CHECK(attending IN ('DEFAULT', 'YES', 'NO'))
);
--> statement-breakpoint
INSERT INTO `__new_event_attendance`("id", "guest_id", "event_id", "attending") SELECT "id", "guest_id", "event_id", CASE WHEN "attending" = 1 THEN 'YES' WHEN "attending" = 0 THEN 'NO' ELSE 'DEFAULT' END FROM `event_attendance`;--> statement-breakpoint
