...
export async function createDbWithSeed(d1Database: D1Database) {
  ...
  await seedEvents(db);
  await seedGuests(db, secondSend);
...
export async function seedGuests(db: Database, csv: string) {
